...
class MainRecyclerAdapter(layoutResId: Int, data: MutableList<MainRecyclerBean.Theme>?, context: Context) :
    ...
    override fun convert(helper: BaseViewHolder?, item: MainRecyclerBean.Theme?) {  //应添加数据判断
        ...
        GlideUtil.LoadCircleImage(mContexts,item?.theme_image_url,helper!!.getView(R.id.head_iv)) //填入主图
        ...
        GlideUtil.LoadCircleImage(mContexts,item!!.theme_goods_list[0].goods_thumbnail_url,helper!!.getView(R.id.left_iv)) //填入左图
        ...
        GlideUtil.LoadCircleImage(mContexts,item!!.theme_goods_list[1].goods_thumbnail_url,helper!!.getView(R.id.right_iv)) //填入右图
    }
