...
class SiteListAdapter() :
    BaseQuickAdapter<SiteBean.Addresse, BaseViewHolder>(R.layout.item_site_list) {
...
//            ?.setText(R.id.siteDet,"${item?.city}  ${item?.address_detail}")
        ...
        if (defStr.contentEquals(item?.address_id.toString())) {
            helper?.setText(
                R.id.siteDet,
                TextAndPictureUtil.getText(
                    mContext,
                    "${if (item?.province != null) item?.province else ""} ${if (item?.city != null) item?.city else ""} ${if (item?.county != null) item?.county else ""} ${item?.address_detail}",
                    R.drawable.sitedef
                )
            )
        } else {
